...
describe("resolveBriefingWindow: DST boundaries", () => {
  it("spring forward: Sunday 2026-03-08 evening opens the Monday 03-09 window at 20:00 New York time", () => {
    ...
    assert.equal(w.auto_show, true);
    assert.equal(w.handover, "weekend");
  });

  it("fall back: Sunday 2026-11-01 evening opens the Monday 11-02 window at 20:00 New York time", () => {
    const w = resolveBriefingWindow(et("2026-11-01", 21, 0));
...
describe("resolveBriefingWindow: weekends", () => {
  it("Saturday noon: hands over to Monday, and does not show itself", () => {
    const w = resolveBriefingWindow(et("2026-09-19", 12, 0));
    assert.equal(w.target_session_ymd, "2026-09-21");
    assert.equal(w.prev_session_ymd, "2026-09-18");
    assert.equal(w.phase, "between_sessions");
    assert.equal(w.auto_show, false);
    assert.equal(w.handover, "weekend");
  ...
  it("Sunday 19:59 ET is still between sessions; 20:00 ET opens the window", () => {
    const before = resolveBriefingWindow(et("2026-09-20", 19, 59));
    assert.equal(before.target_session_ymd, "2026-09-21");
    assert.equal(before.phase, "between_sessions");
    assert.equal(before.auto_show, false);

    const at = resolveBriefingWindow(et("2026-09-20", 20, 0));
    assert.equal(at.target_session_ymd, "2026-09-21");
    assert.equal(at.phase, "pre_open");
    assert.equal(at.auto_show, true);
  });

  it("Monday 03:30 ET: pre-open, shows itself, weekend handover", () => {
    const w = resolveBriefingWindow(et("2026-09-21", 3, 30));
    assert.equal(w.target_session_ymd, "2026-09-21");
    assert.equal(w.prev_session_ymd, "2026-09-18");
    assert.equal(w.phase, "pre_open");
    assert.equal(w.auto_show, true);
...
describe("resolveBriefingWindow: holidays and early closes", () => {
  it("Labor Day 2026-09-07 morning: target Tuesday, not showing yet, holiday handover", () => {
    const w = resolveBriefingWindow(et("2026-09-07", 8, 0));
    assert.equal(w.target_session_ymd, "2026-09-08");
    assert.equal(w.prev_session_ymd, "2026-09-04");
    assert.equal(w.handover, "holiday");
    assert.equal(w.phase, "between_sessions");
    assert.equal(w.auto_show, false);
  ...
  it("Labor Day 2026-09-07 evening: same target, now pre-open", () => {
    const w = resolveBriefingWindow(et("2026-09-07", 21, 0));
    assert.equal(w.target_session_ymd, "2026-09-08");
    assert.equal(w.prev_session_ymd, "2026-09-04");
    assert.equal(w.handover, "holiday");
    assert.equal(w.phase, "pre_open");
  ...
  it("Thanksgiving 2026-11-26: hands over to the early-close Friday", () => {
    const w = resolveBriefingWindow(et("2026-11-26", 10, 0));
    ...
    const evening = resolveBriefingWindow(et("2026-11-26", 20, 0));
  ...
  it("early-close Friday 2026-11-27: in session until 13:00 ET, then Monday with overnight starting at 13:00", () => {
    const during = resolveBriefingWindow(et("2026-11-27", 12, 59));
    ...
    const after = resolveBriefingWindow(et("2026-11-27", 13, 0));
    ...
    assert.equal(after.handover, "weekend");
    assert.equal(after.phase, "between_sessions");
  });

  it("a Friday holiday joined to the weekend reads as a holiday handover, not a weekend", () => {
    // Juneteenth 2026 is a Friday: Thursday close to Monday open spans the holiday and the weekend.
    const w = resolveBriefingWindow(et("2026-06-20", 12, 0));
...
describe("resolveBriefingWindow: inside a trading day", () => {
  it("00:30 ET: the window opened the evening before, target is today", () => {
    const w = resolveBriefingWindow(et("2026-09-22", 0, 30));
    ...
    assert.equal(w.phase, "pre_open");
    assert.equal(w.auto_show, true);
    assert.equal(w.handover, "overnight");
    assert.equal(w.early_close, false);
  });

  it("09:29 ET is pre-open; 09:30 ET is in session and no longer shows itself", () => {
    const before = resolveBriefingWindow(et("2026-09-22", 9, 29));
    assert.equal(before.phase, "pre_open");
    assert.equal(before.auto_show, true);

    const at = resolveBriefingWindow(et("2026-09-22", 9, 30));
    assert.equal(at.target_session_ymd, "2026-09-22");
    assert.equal(at.phase, "in_session");
    assert.equal(at.auto_show, false);
  ...
  it("the close flips the target: 15:59 ET is today, 16:00 ET is the next session", () => {
    const before = resolveBriefingWindow(et("2026-09-22", 15, 59));
    assert.equal(before.target_session_ymd, "2026-09-22");
    assert.equal(before.phase, "in_session");

    const at = resolveBriefingWindow(et("2026-09-22", 16, 0));
    assert.equal(at.target_session_ymd, "2026-09-23");
    assert.equal(at.phase, "between_sessions");
  });

  it("17:00 ET: between sessions, handing over to the next trading day", () => {
    const w = resolveBriefingWindow(et("2026-09-22", 17, 0));
  ...
  it("Friday 17:00 ET hands over to Monday across the weekend", () => {
    const w = resolveBriefingWindow(et("2026-09-18", 17, 0));
    assert.equal(w.target_session_ymd, "2026-09-21");
    assert.equal(w.prev_session_ymd, "2026-09-18");
    assert.equal(w.handover, "weekend");
    assert.equal(w.window_opens_at, iso("2026-09-20", 20, 0));
...
describe("resolveBriefingWindow: invariants", () => {
  it("hold for every half hour across both DST changes, Thanksgiving and the year end", () => {
    ...
    for (const [from, to] of ranges) {
      ...
      for (let ms = et(from, 0, 0).getTime(); ms < end; ms += 30 * 60_000) {
        ...
        // The report never describes a session that is already over, and
        ...
        assert.equal(w.auto_show, w.phase === "pre_open", label);
  ...
  it("rejects an invalid clock instead of resolving a window from it", () => {
    assert.throws(() => resolveBriefingWindow(new Date(Number.NaN)), RangeError);
  });
});

describe("resolveBriefingWindow: ad-hoc session overrides", () => {
  ...
  it("rolls the target forward once an added early close has passed", () => {
    ...
    const plain = resolveBriefingWindow(at);
    assert.equal(plain.target_session_ymd, "2026-09-21");
    assert.equal(plain.phase, "in_session");

    const w = resolveBriefingWindow(at, override("2026-09-21", true));
    assert.equal(w.target_session_ymd, "2026-09-22");
    assert.equal(w.prev_session_ymd, "2026-09-21");
    ...
    // Before that close the report is still on the session itself.
    const earlier = resolveBriefingWindow(et("2026-09-21", 11, 0), override("2026-09-21", true));
  ...
  it("keeps the target on a still-trading session when a wrongly derived early close is cleared", () => {
    ...
    assert.equal(resolveBriefingWindow(at).target_session_ymd, "2026-11-30");

    const w = resolveBriefingWindow(at, override("2026-11-27", false));
    assert.equal(w.target_session_ymd, "2026-11-27");
    assert.equal(w.early_close, false);
  ...
  it("ignores a row that is not one, and one for a day it does not touch", () => {
    ...
    assert.deepEqual(resolveBriefingWindow(at, junk), plain);
    assert.deepEqual(resolveBriefingWindow(at, override("2026-09-14", true)), plain);
    assert.deepEqual(resolveBriefingWindow(at, undefined), plain);
  });
});
