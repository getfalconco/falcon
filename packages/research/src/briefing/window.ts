/**
 * Handover briefing: which US session a report hands over to, and where the
 * reader stands relative to it.
 *
 * Pure: the clock is a parameter, and every date rule (holidays, early closes,
 * DST) comes from the Tracker's market calendar, so the briefing and the chain
...
import type { BriefingHandover, BriefingPhase, BriefingWindow, SessionOverride } from "./types.js";
 ...
 * before (not the previous trading day) is what makes a Monday report open on
 * Sunday evening instead of Friday evening, when nothing has traded yet.
 */
const WINDOW_OPENS_HOUR_ET = 20;

/**
 * A gap longer than one night is a "holiday" handover as soon as one closed
 ...
 */
function classifyHandover(prevYmd: string, targetYmd: string): BriefingHandover {
 ...
 * afterwards leaves the report handing over to a session that ended hours ago
 * when a close is added, and to tomorrow while today is still trading when a
 * wrongly derived one is cleared. An override moves a close and nothing else;
 * it never makes a day trade or stop trading.
 */
export function resolveBriefingWindow(now: Date, overrides: readonly SessionOverride[] = []): BriefingWindow {
  const t = now.getTime();
  // Intl would throw an opaque RangeError a few frames down; a bad clock is a
  // caller bug and is named as one here.
  if (!Number.isFinite(t)) throw new RangeError("resolveBriefingWindow: invalid clock");
  ...
  // 11:00 still gets today's report (phase "in_session"), and only after the
  // bell does the report start describing tomorrow.
  const target =
  ...
  if (!targetTimes || !prevTimes) {
    throw new Error(`resolveBriefingWindow: no session times for ${prev} / ${target}`);
  }
  ...
  return {
    ...
    phase,
    auto_show: phase === "pre_open",
    handover: classifyHandover(prev, target),
