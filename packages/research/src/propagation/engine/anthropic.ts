/**
 * The real `ModelCaller` for stage-2: one Anthropic Messages call with
 * structured JSON output, bounded max_tokens and a per-call timeout/abort.
 * Never imported by tests — they inject fixture callers.
 *
 * Same Fable 5 shape as the Analyst's caller: no sampling parameters, depth
 * via `output_config.effort`, and a safety refusal (`stop_reason: "refusal"`)
 * surfaced as a non-retryable error so the run ships stage-1-only instead of
 * burning retries.
 */
...
function getClient(): Anthropic {
  ...
  const route = providerRouteFor("propagation");
  const currentKey = route.apiKey;
  if (client && clientKey === currentKey) return client;
  const apiKey = route.apiKey;
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is not configured");
  // SDK retries are disabled: the service owns retry/backoff/breaker so the
...
export const anthropicModelCaller: ModelCaller = async (input) => {
  const response = await withProviderSlot("propagation", () =>
