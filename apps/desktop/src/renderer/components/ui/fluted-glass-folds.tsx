...
// parent. Ported from apps/web for the desktop login panel.
...
const UNIFORMS = {
  colors: [
    [0.403921568627451, 0.9098039215686274, 0.9764705882352941],
    [0.08235294117647059, 0.3686274509803922, 0.4588235294117647],
    [0.027450980392156862, 0.10196078431372549, 0.1411764705882353],
    [0.9411764705882353, 0.9921568627450981, 0.9803921568627451],
    [0.9411764705882353, 0.9921568627450981, 0.9803921568627451],
    [0.9411764705882353, 0.9921568627450981, 0.9803921568627451],
    [0.9411764705882353, 0.9921568627450981, 0.9803921568627451],
    [0.9411764705882353, 0.9921568627450981, 0.9803921568627451],
  ...
  cursorRadius: 0.46,
  oklab: 0.0,
  // ~6.25× the site's base speed (1.316).
  timeScale: 8.225,
};
...
export function ShaderBackground({
  ...
  className?: string;
  /** Override the palette (RGB 0..1). Pass a stable reference (module const). */
  colors?: [number, number, number][];
  colorCount?: number;
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Latest palette/count, read fresh every frame so color-picker edits apply live
  // without tearing down the WebGL context.
  const propsRef = useRef({ colors, colorCount });
  useEffect(() => {
    propsRef.current = { colors, colorCount };
  }, [colors, colorCount]);
  ...
  useEffect(() => {
    ...
    const uploadColors = () => {
      const palette = propsRef.current.colors ?? UNIFORMS.colors;
      for (let i = 0; i < 8; i++) {
        const c = palette[i] ?? palette[palette.length - 1] ?? [1, 1, 1];
        colorBuf[i * 3] = c[0];
        colorBuf[i * 3 + 1] = c[1];
        colorBuf[i * 3 + 2] = c[2];
      }
    ...
    const activeColorCount = () => {
      ...
      return p.colorCount ?? (p.colors ? p.colors.length : UNIFORMS.colorCount);
    };
    ...
    gl.uniform3fv(uni.colors, uploadColors());
