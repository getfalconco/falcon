...
interface DockApp {
  ...
  name: string;
  icon: string;
  /** Decorative entry: still magnifies, but ignores clicks (no bounce, no callback). */
  inert?: boolean;
}
...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  // Start from the SSR default and only measure the viewport after mount, so
  // the server and client first paints match (no hydration mismatch).
  const [config, setConfig] = useState({ baseIconSize: 64, maxScale: 1.6, effectWidth: 240 });
  const { baseIconSize, maxScale, effectWidth } = config;
  const minScale = 1.0;
  const baseSpacing = Math.max(4, baseIconSize * 0.08);

  // Measure once after mount, then keep up with window resizes
  useEffect(() => {
    const handleResize = () => {
        ...
        {apps.map((app, index) => {
          ...
          return (
            <div
              ...
              className={`absolute ${app.inert ? 'cursor-default' : 'cursor-pointer'} flex flex-col items-center justify-end`}
              title={app.name}
