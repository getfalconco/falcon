...
import { FONTS, PRODUCT, TICKET } from "@/theme";

/**
 * RN port of the web Awards `variant="award"` laurel (shadcn/Tailwind award.tsx).
 * Paths copied from the reference component viewBox 0 0 892.77 688.08.
 */
const LAUREL_RIGHT =
...
/** Matches award.tsx levelColors (+ Early Access for founding members). */
const RANK_STYLES: Record<string, RankStyle> = {
  early: {
    label: "Early Access",
    colors: [TICKET.back, TICKET.back],
    text: TICKET.ink,
  },
  bronze: {
    label: "Bronze",
  ...
  staff: {
    label: "Staff",
    colors: ["#2f7d4f", "#8ee6a8"],
    text: "#ffffff",
  },
  deputy: {
    label: "Deputy",
    colors: ["#1e3a5f", "#6ba3d4"],
    text: "#ffffff",
  },
  leader: {
    label: "Leader",
    colors: ["#4a1d6a", "#c084fc"],
    text: "#ffffff",
  },
};

export function membershipRankFor(user: User | null | undefined): RankStyle {
  ...
  if (raw === "early-access" || raw === "early_access" || raw === "founding") {
    return RANK_STYLES.early;
  }
  return RANK_STYLES[raw] ?? RANK_STYLES.early;
}
...
function LaurelWreath({ width }: { width: number }) {
  const height = width * (688.08 / 892.77);
  return (
    <Svg
      width={width}
      height={height}
      ...
      pointerEvents="none"
    >
      <Path d={LAUREL_LEFT} fill={PRODUCT.fg} />
      <Path d={LAUREL_RIGHT} fill={PRODUCT.fg} />
    </Svg>
  );
}
...
type Props = {
  user: User | null;
  date?: Date;
  /** Maps to Awards `title` */
  title?: string;
  /** Maps to Awards `subtitle` */
  subtitle?: string;
};

/**
 * Mobile port of Awards variant="award" for the Signals empty state.
 * Membership rank → level badge; user name → recipient; today → date.
 */
export default function SignalsClearCertificate({
  user,
  date,
  title = "CLEAR DESK",
  subtitle = "No high-confidence window worthy of your attention today.",
}: Props) {
  const rank = useMemo(() => membershipRankFor(user), [user]);
  const recipient = useMemo(() => displayName(user), [user]);
  const dateLabel = formatCertificateDate(date ?? new Date());

  return (
    <View style={styles.wrap} accessibilityRole="text">
      <View style={styles.frame}>
        <LaurelWreath width={WREATH_W} />
        <View style={styles.inner}>
          <LinearGradient
            ...
            style={styles.badge}
          >
            <Text style={[styles.badgeText, { color: rank.text }]} numberOfLines={1}>
...
const WREATH_W = 240;
const WREATH_H = WREATH_W * (688.08 / 892.77);
