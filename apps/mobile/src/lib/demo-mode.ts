/**
 * Demo / presentation mode — desktop Ctrl/⌘+P parity.
 *
 * While on, the home dashboard shows a randomly generated portfolio (five- or
 * six-figure balance, random holdings, random growth curve) instead of the
 * real Falcon paper account. Nothing is written to Supabase or AsyncStorage:
 * toggling off or reloading restores the real data untouched.
 */

import { useEffect, useState } from "react";
import type { PaperAccount, PaperPosition } from "@/lib/brokers";
...
export function toggleDemoMode(): boolean {
  snapshot = snapshot
    ? null
    : generate((Date.now() ^ ((Math.random() * 0xffffffff) >>> 0)) >>> 0);
  notify();
  return snapshot != null;
}
...
export type DemoSeriesPoint = { t: number; value: number };

/**
 * Random-walk-with-drift value curve over `days`, scaled so it ends exactly
 * at `endValue` (the live demo balance) — the headline and the chart agree.
 */
export function demoValueSeries(
  seed: number,
  days: number,
  endValue: number,
): DemoSeriesPoint[] {
...
/** React view of the demo state — re-renders when + / Ctrl+P toggles. */
